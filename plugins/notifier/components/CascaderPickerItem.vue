<template>
  <div class="pd-select-item">
    ...
    <div class="pd-select-list" v-if="renderData">
    ...
    <ul class="pd-select-wheel" ref="wheel">
      <li class="pd-select-wheel-item" :class="{'hidden':setHidden(el.index)}"
          :style="setWheelItemDeg(el.index)" :index="el.index"
          v-for="el,index in renderData " :key="index">{{el.value}}
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  computed: {
    renderData () {
      const vm = this
      let temp = []
      if (!vm.listData.length) return temp
      for (let k = vm.spin.start; k <= vm.spin.end; k++) {
        let data = {
          value: vm.getSpinData(k).text,
          index: k
        }
        temp.push(data)
      }
  ...
  mounted () {
    ...
    const vm = this
    window.iii = vm
    vm.$el.addEventListener('touchstart', vm.itemTouchStart)
    vm.$el.addEventListener('touchmove', vm.itemTouchMove)
    vm.$el.addEventListener('touchend', vm.itemTouchEnd)
    vm.init()
  },
  methods: {
    /* 初始化状态 */
    ...
    setHidden (index) {
      const vm = this
      if (vm.type === 'line') {
        return index < 0 || index > vm.listData.length - 1
      } else {
        return false
      }
    },
    ...
    setStyle (move, type, time) {
      ...
      let currentListMove = vm.finger.transformY
      let updateMove = move + Number(currentListMove)
      let updateDeg, spinAim, margin, endMove, endDeg
      if (type === 'end' && vm.type === 'line') {
        /*这里只在释放的时候判断 实现缓动效果*/
        /* 根据滚轮类型 line or cycle 判断 updateMove最大距离 */
        if (updateMove > 0) {
          updateMove = 0
        }
        if (updateMove < -(vm.listData.length - 1) * singleHeight) {
          updateMove = -(vm.listData.length - 1) * singleHeight
        }
      }
    ...
    getSpinData (index) {
      const vm = this
      index = index % vm.listData.length
      index += index < 0 ? vm.listData.length : 0
      return vm.listData[index]
    },
    /* 获取选中值 */
    getPickValue (move) {
      const vm = this
      if (!vm.listData.length) return void 0
      let index = Math.round(-move / 34)
      let pickValue = vm.getSpinData(index).value
      vm.$emit('input', pickValue)
    }
  },
  beforeDestroy () {
